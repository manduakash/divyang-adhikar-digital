// app/dashboard/nabd/layout.tsx

import { NABDSidebar } from "@/components/layout/central/NABDSidebar";
import { NABDHeader } from "@/components/layout/central/NABDHeader";

export default function NABDLayout({ children }: { children: React.ReactNode }) {
    return (
        <div className="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans selection:bg-indigo-100">
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                ...
                {/* TOP: ADVISORY COMMAND HEADER */}
                <NABDHeader />

                {/* CENTER: NABD WORKSPACE CANVAS */}
                ...
                {/* BOTTOM: GOVERNANCE ACCOUNTABILITY STRIP */}
                <footer className="h-10 bg-indigo-950 border-t border-white/5 flex items-center justify-between px-8 z-20 text-white">
                    <div className="flex items-center gap-6">
                        <span className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">
                            Board ID: <span className="text-white font-mono">NABD-SEC-001-APEX</span>
                        </span>
                        <div className="h-3 w-[1px] bg-white/10" />
                        <div className="flex items-center gap-2">
                            <div className="h-1.5 w-1.5 rounded-full bg-blue-500 animate-pulse" />
                            <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">
                                Advisory Governance Sync: ACTIVE
                            </span>
                        </div>
                    </div>

                    <div className="flex gap-8 text-[9px] font-black text-indigo-400 uppercase tracking-widest">
                        <button className="hover:text-white transition-colors">PwD Act 2016 (Policy)</button>
                        <button className="hover:text-white transition-colors">Board Delegation Framework</button>
                        <span className="opacity-30">v4.1.0-NABD-Core</span>
                    </div>
                </footer>
