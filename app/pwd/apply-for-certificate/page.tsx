          ...
          <CardContent className="p-8 space-y-8">
            ...
            <div className="space-y-3">
                ...
                <Textarea 
                    ...
                    className="rounded-2xl border-none bg-slate-50 p-4 focus-visible:ring-blue-600 min-h-[120px]"
                />
            </div>
          </CardContent>
        </Card>
...
function UploadSlot({ label, desc, uploaded }: { label: string, desc: string, uploaded: boolean }) {
    return (
        <div className={`relative p-6 rounded-[24px] border-2 border-dashed transition-all flex flex-col items-center text-center gap-3 group cursor-pointer ${
            uploaded ? 'border-green-200 bg-green-50/50' : 'border-slate-200 bg-slate-50 hover:bg-white hover:border-blue-300'
        }`}>
            <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shadow-sm transition-colors ${
                uploaded ? 'bg-white text-green-600' : 'bg-white text-slate-400 group-hover:text-blue-600'
            }`}>
            ...
            {uploaded && (
                <Badge className="absolute -top-2 -right-2 bg-green-600 text-white border-none text-[8px] font-black uppercase">Ready</Badge>
            )}
        </div>
    );
}